    ...
    IEnumerator UpdatePeople()
    {
        ...
        while (true)
        {
            if (Hologram1.activeSelf){
                DataEntry entry = dataList.Find(e => e.ID_Building == 1 && e.ID_DataType == 5 && e.ID_Floor == TemperatureDisplay.currentFloor && e.Data == "19/10/23" && e.Time == time);
                if (entry != null)
                {
                    peopleText.text = "" + entry.Value;
                }
                else
                {
