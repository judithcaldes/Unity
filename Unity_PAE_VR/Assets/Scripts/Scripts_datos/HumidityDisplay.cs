...
public class HumidityDisplay : MonoBehaviour
{
        ...
        while (true)
        {
            if (Hologram1.activeSelf){
                DataEntry entry = dataList.Find(e => e.ID_Building == 1 && e.ID_Floor == TemperatureDisplay.currentFloor &&
                                                 e.ID_DataType == 9 && e.Data == "19/10/23" && e.Time == time);
                if (entry != null)
            {
                humidityText.text = "" + entry.Value;
            }
            else
            {
                humidityText.text = "No data found";
            }
            }
