...
public class AtmosphericPressureDisplay : MonoBehaviour
{
    public Text apText;
        ...
        while (true)
        {
            if (Hologram1.activeSelf)
            {
                DataEntry entry = dataList.Find(e => e.ID_Building == 1 && e.ID_Floor == TemperatureDisplay.currentFloor &&
                                                 e.ID_DataType == 10 && e.Data == "19/10/23" && e.Time == time);
                if (entry != null)
                {
                    apText.text = "" + entry.Value;
                }
                else
                {
                    apText.text = "No data found";
                }
            }
