...
public class SolarPanelsDisplay : MonoBehaviour
{
    public Text solarpanelsText;
    ...
    private IEnumerator Start()
    {
        ...
        StartCoroutine(UpdateSolarPanels());
    }
        ...
        while (true)
        {
            if (Hologram1.activeSelf){
                DataEntry2 entry = dataList.Find(e => e.ID_Building == 1 &&
                                                 e.ID_TypeData == 6 && e.Data == "19/10/23" && e.Time == time);
                if (entry != null)
                {
                    solarpanelsText.text = "" + entry.Value;
                }
                else
                {
                    solarpanelsText.text = "No data found";
                }
            }
