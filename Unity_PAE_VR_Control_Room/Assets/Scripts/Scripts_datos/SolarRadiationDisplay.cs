...
public class SolarRadiationDisplay : MonoBehaviour
{
    public Text solarradiationText;
    ...
    private IEnumerator Start()
    {
        ...
        StartCoroutine(UpdateSolarRadiation());
    }
    ...
    IEnumerator UpdateSolarRadiation()
    {
        int time = 0; 
        ...
        while (true)
        {
            if (Hologram1.activeSelf){
                DataEntry2 entry = dataList.Find(e => e.ID_Building == 1 &&
                                                 e.ID_TypeData == 7 && e.Data == "19/10/23" && e.Time == time);
                if (entry != null)
            {
                solarradiationText.text = "" + entry.Value;
            }
            else
            {
                solarradiationText.text = "No data found";
            }
            }
