        ...
        while (true)
        {
            if (Hologram1.activeSelf){
                DataEntry2 entry = dataList.Find(e => e.ID_Building == 1 &&
                                                 e.ID_TypeData == 13 && e.Data == "19/10/23" && e.Time == time);
                if (entry != null)
            {
                daylightText.text = "" + entry.Value;
            }
            else
            {
                daylightText.text = "No data found";
            }
            }
